...
const CHART_DATA = [
  ...
  { day: 'THU', val: 90 }, { day: 'FRI', val: 70 }, { day: 'SAT', val: 30 }, { day: 'SUN', val: 55 },
]
const ACTIVITY = [
...
function Clock() {
  ...
  return <span className="dash__header-time">{time.toLocaleTimeString('en-US', { hour12: false })}</span>
}
        ...
        {STATS.map((s, i) => (
          <div key={i} className={`dash__stat dash__stat--${s.color}`}>
            ...
            <div className="dash__stat-bar">
              <div className={`dash__stat-bar-fill dash__stat-bar-fill--${s.color}`}
      ...
      <div className="dash__table-wrap">
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="dash__table">
            <thead>
              <tr><th>ID</th><th>PROJECT</th><th>STATUS</th><th>DATE</th><th>AMOUNT</th></tr>
            </thead>
            <tbody>
...
function PageComingSoon({ title }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: 16 }}>
      <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 80, color: '#1e1e1e', letterSpacing: 4 }}>
        {title.toUpperCase()}
      </div>
      <div style={{ fontSize: 11, color: '#444', letterSpacing: 3 }}>// COMING SOON</div>
    </div>
  )
}
...
export default function DashboardPage() {
  ...
  const renderPage = () => {
    switch (active) {
      case 'dashboard': return <PageDashboard authUser={authUser} />
      ...
      default:          return <PageComingSoon title={active} />
    }
  }
    ...
    <div className="dash">
      <aside className={`dash__sidebar ${sideOpen ? 'dash__sidebar--open' : ''}`}>
        ...
        <div className="dash__user">
          <div className="dash__user-avatar">{getInitials(authUser.name)}</div>
      ...
      <div className="dash__main">
        ...
        <div className="dash__statusbar">
          <div className="dash__status-item"><span className="dash__status-dot" /><span>ALL SYSTEMS OPERATIONAL</span></div>
