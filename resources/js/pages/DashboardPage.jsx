...
const NAV = [
    {
        ...
        items: [
            ...
            { icon: "◈", label: "PROJECTS", badge: null, id: "projects" },
    ...
    },
    {
        ...
        items: [
            ...
            { icon: "◷", label: "SCHEDULE", badge: null, id: "schedule" },
...
const ACTIVITY = [
    {
        color: "red",
        text: "New deployment pushed to production",
        sub: "nexus-api v2.6.1",
        time: "2m ago",
    },
    {
        color: "green",
        text: "User registration spike detected",
        sub: "+142 signups today",
        time: "15m ago",
    },
    {
        color: "yellow",
        text: "Scheduled backup completed",
...
function PageDashboard({ authUser }) {
        ...
        <>
            <div className="dash__page-header">
                ...
                <div className="dash__header-actions">
                    <button className="dash__btn dash__btn--ghost">
                        EXPORT
                    </button>
            ...
            <div className="dash__stats">
                {STATS.map((s, i) => (
                    <div
                        key={i}
                        className={`dash__stat dash__stat--${s.color}`}
                    >
                        ...
                        <div
                            className={`dash__stat-change dash__stat-change--${s.dir}`}
                        >
                            {s.dir === "up" ? "▲" : "▼"} {s.change} vs last
                            month
                        </div>
            ...
            <div className="dash__grid">
                <div className="dash__panel">
                    <div className="dash__panel-header">
                        ...
                        <button className="dash__panel-action">
                            VIEW ALL →
                        </button>
                    </div>
                        ...
                        <div className="dash__quick">
                            {QUICK.map((q, i) => (
                                <div className="dash__quick-item" key={i}>
                                    <div style={{ flex: 1 }}>
                                        <div className="dash__quick-label">
                                            {q.label}
                                        </div>
                        ...
                        <tbody>
                            {TABLE_ROWS.map((row, i) => (
                                <tr key={i}>
                                    ...
                                    <td>
                                        <span
                                            className={`dash__badge dash__badge--${row.status}`}
                                        >
                                            {row.status}
                                        </span>
                                    </td>
                    ...
                    <div className="dash__activity">
                        {ACTIVITY.map((a, i) => (
                            <div className="dash__activity-item" key={i}>
                                <div
                                    className={`dash__activity-dot dash__activity-dot--${a.color}`}
                                />
...
export default function DashboardPage() {
    ...
    const renderPage = () => {
        switch (active) {
            case "dashboard":
                return <PageDashboard authUser={authUser} />;
            case "projects":
                return <PageProjects />;
            case "analytics":
                return <PageAnalytics />;
            // case 'team':      return <PageTeam />
            // case 'settings':  return <PageSettings authUser={authUser}
            default:
                return <PageComingSoon title={active} />;
        }
    };
    console.log("bulu", active);

    return (
        <div className="dash">
            <aside
                className={`dash__sidebar ${sideOpen ? "dash__sidebar--open" : ""}`}
            >
                        ...
                        <div className="dash__nav-section" key={group.section}>
                            ...
                            {group.items.map((item) => (
                                ...
                                >
                                    ...
                                    <span className="dash__nav-text">
                                        {item.label}
                                    </span>
                                    {item.badge && (
                                        <span className="dash__nav-badge">
                                            {item.badge}
                                        </span>
                                    )}
                                </div>
            ...
            <div className="dash__main">
                <header className="dash__header">
                    ...
                    <div className="dash__header-right">
                        <button
                            className={`dash__theme-toggle ${theme === "light" ? "dash__theme-toggle--light" : ""}`}
                            onClick={() =>
                                setTheme((t) =>
                                    t === "dark" ? "light" : "dark",
                                )
                        ...
                        >
                            <div className="dash__theme-toggle__track">
                                <span className="dash__theme-toggle__icon dash__theme-toggle__icon--moon">
                                    ☽
                                </span>
                                <span className="dash__theme-toggle__icon dash__theme-toggle__icon--sun">
                                    ☀
                                </span>
                                <div className="dash__theme-toggle__knob" />
                            </div>
                        </button>
                        <Clock />
                        <button className="dash__header-btn dash__header-btn--notif">
                            ◎
                        </button>
                ...
                <div className="dash__content" key={active}>
                    {renderPage()}
                </div>
