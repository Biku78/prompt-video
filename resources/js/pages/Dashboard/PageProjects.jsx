...
const STATUS_CONFIG = {
  completed:  { color: '#22c55e', bg: 'rgba(34,197,94,0.1)',   border: 'rgba(34,197,94,0.25)',  label: '✓ Completed',  dot: '#22c55e' },
  processing: { color: '#6366f1', bg: 'rgba(99,102,241,0.1)',  border: 'rgba(99,102,241,0.3)',  label: '⟳ Processing', dot: '#6366f1' },
  pending:    { color: '#f59e0b', bg: 'rgba(245,158,11,0.1)',  border: 'rgba(245,158,11,0.25)', label: '◎ Pending',    dot: '#f59e0b' },
  failed:     { color: '#ef4444', bg: 'rgba(239,68,68,0.1)',   border: 'rgba(239,68,68,0.25)',  label: '✕ Failed',     dot: '#ef4444' },
}
...
A 3D animation of a warehouse with boxes labeled LET and CONST.`
  ...
  const handleGenerate = async () => {
    if (!script.trim()) { setError('Please enter your script first.'); return }
    setError('')
    setLoading(true)
    try {
      ...
      if (!data.success) { setError(data.message || 'Failed to start generation.'); return }
      onGenerated(data)
      onClose()
    } catch (e) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            <div style={{ fontSize: 12, color: '#6366f1', letterSpacing: 2, marginBottom: 6, textTransform: 'uppercase', fontFamily: 'Syne, sans-serif' }}>
              AI VIDEO STUDIO
            </div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800, color: '#f1f5f9' }}>
              Generate New Video
            </h2>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          <label style={{ fontSize: 13, color: '#64748b' }}>Voiceover Language:</label>
          <select value={language} onChange={e => setLanguage(e.target.value)} style={{
            background: '#0e1422', border: '1px solid #1e2a3a', color: '#f1f5f9',
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <label style={{ fontSize: 13, color: '#64748b' }}>Your Script</label>
            <button onClick={() => setScript(EXAMPLE)} style={{
          ...
          <textarea
            ...
            rows={10}
            style={{
              width: '100%', background: '#0e1422', border: '1px solid #1e2a3a',
              borderRadius: 10, color: '#f1f5f9', padding: 16,
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
          {['🆓 100% Free', '🎙️ Hindi Voice', '🎬 Auto Video', '⏱️ ~20 min/video'].map((chip, i) => (
            <span key={i} style={{
              background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)',
              color: '#a5b4fc', padding: '4px 12px', borderRadius: 20, fontSize: 12,
            }}>{chip}</span>
          ))}
        </div>
        ...
        {error && (
          <div style={{
            ...
            color: '#fca5a5', padding: '12px 16px', borderRadius: 8, fontSize: 13, marginBottom: 16,
          }}>⚠️ {error}</div>
        )}
        ...
        <button
          ...
          style={{
            ...
            background: loading || !script.trim()
              ? '#1e2a3a'
...
function VideoCard({ video, onDelete }) {
  ...
  return (
    <div style={{
      ...
      onMouseLeave={e => {
        e.currentTarget.style.borderColor = isProcessing ? cfg.border : '#1e2a3a'
      ...
      <div style={{
        ...
        {video.status === 'completed' && video.final_video_url ? (
          <video
            ...
            src={`http://localhost:8000${video.final_video_url}`}
            muted
          ...
          <div style={{
            ...
            {isProcessing && (
              <div style={{ fontSize: 12, color: cfg.color, fontFamily: 'Syne, sans-serif' }}>
      ...
      <div style={{ padding: '16px 18px' }}>
        ...
        {isProcessing && (
          <div style={{ fontSize: 12, color: '#64748b', marginBottom: 10 }}>
            Scene {video.scenes_done ?? 0} of {video.scenes_total ?? '?'} — {video.progress ?? 0}% complete
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 14, marginBottom: 14, flexWrap: 'wrap' }}>
          {[
            ...
            <span key={i} style={{ fontSize: 12, color: '#64748b' }}>{meta}</span>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {video.status === 'completed' && video.final_video_url && (
            <a
              href={`http://localhost:8000${video.final_video_url}`}
          ...
          <button
            ...
            onMouseLeave={e => { e.target.style.borderColor = '#1e2a3a'; e.target.style.color = '#64748b'; e.target.style.background = 'transparent' }}
          >
            🗑 Delete
          </button>
        </div>
...
export default function PageProjects() {
  ...
  const pollInProgress = async (videoList) => {
    const inProgress = videoList.filter(v => ['processing', 'pending'].includes(v.status))
    if (!inProgress.length) return

    for (const video of inProgress) {
      try {
        const token = localStorage.getItem('auth_token')
        const res = await fetch(`${API_BASE}/video-status/${video.job_id}`, {
          headers: {
            'Accept': 'application/json',
            ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
          },
  ...
  useEffect(() => {
    const hasActive = videos.some(v => ['processing', 'pending'].includes(v.status))
    if (hasActive) {
  ...
  const stats = {
    ...
    processing: videos.filter(v => ['processing', 'pending'].includes(v.status)).length,
  ...
  return (
    <>
      ...
      <div className="dash__page-header">
        <div>
          <div className="dash__page-eyebrow" style={{ color: '#6366f1' }}>AI VIDEO STUDIO</div>
        ...
        <div className="dash__header-actions">
          <button className="dash__btn dash__btn--ghost" onClick={fetchVideos}>↻ Refresh</button>
          <button
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Total Videos',  value: stats.total,      color: '#f1f5f9', bg: 'rgba(241,245,249,0.05)' },
          { label: 'Completed',     value: stats.completed,  color: '#22c55e', bg: 'rgba(34,197,94,0.07)'   },
          { label: 'Processing',    value: stats.processing, color: '#6366f1', bg: 'rgba(99,102,241,0.07)'  },
          { label: 'Failed',        value: stats.failed,     color: '#ef4444', bg: 'rgba(239,68,68,0.07)'   },
        ].map((s, i) => (
          <div key={i} style={{
            background: s.bg, border: '1px solid #1e2a3a',
            borderRadius: 12, padding: '16px 20px',
          }}>
            <div style={{ fontSize: 11, color: '#64748b', letterSpacing: 1, marginBottom: 8, textTransform: 'uppercase', fontFamily: 'Syne, sans-serif' }}>
              {s.label}
            </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {FILTERS.map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            background: filter === f ? 'linear-gradient(135deg, #6366f1, #ec4899)' : 'transparent',
            border: `1px solid ${filter === f ? 'transparent' : '#1e2a3a'}`,
            color: filter === f ? 'white' : '#64748b',
            fontFamily: 'Syne, sans-serif', fontWeight: filter === f ? 700 : 400,
            fontSize: 13, transition: 'all 0.15s', textTransform: 'capitalize',
          }}
            onMouseEnter={e => { if (filter !== f) { e.target.style.borderColor = '#6366f1'; e.target.style.color = '#a5b4fc' } }}
            onMouseLeave={e => { if (filter !== f) { e.target.style.borderColor = '#1e2a3a'; e.target.style.color = '#64748b' } }}
          >
            {f === 'all' ? `All (${videos.length})` : `${f.charAt(0).toUpperCase() + f.slice(1)} (${videos.filter(v => v.status === f).length})`}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '40vh', gap: 16 }}>
          <div style={{
            ...
            border: '3px solid #1e2a3a', borderTopColor: '#6366f1',
            animation: 'spin 0.8s linear infinite',
          }} />
          <div style={{ color: '#64748b', fontFamily: 'Syne, sans-serif', fontSize: 14 }}>Loading videos...</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '40vh', gap: 16 }}>
          ...
          <div style={{ color: '#64748b', fontSize: 14 }}>
            {filter === 'all' ? 'Generate your first AI video!' : 'Try a different filter'}
          </div>
          {filter === 'all' && (
            <button
              onClick={() => setShowModal(true)}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: 18 }}>
          {filtered.map(video => (
            <VideoCard key={video.job_id} video={video} onDelete={handleDelete} />
          ))}
        </div>
      )}
    </>
